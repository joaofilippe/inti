...
func (r *MandadoRepository) SalvarExtraido(ctx context.Context, m *dto.MandadoExtraido) error {
	...
	_, err := r.db.NamedExecContext(ctx, `
		INSERT INTO mandados (numero, lote, numero_processo, nome, documento, tipo_documento, sexo, posicao, endereco, cidade, situacao, data_carga, tipo_ato, diligencias, updated_at)
		VALUES (:numero, :lote, :numero_processo, :nome, :documento, :tipo_documento, :sexo, :posicao, :endereco, :cidade, :situacao, :data_carga, :tipo_ato, :diligencias, NOW())
...
func (r *MandadoRepository) SalvarMandado(ctx context.Context, m *entities.Mandado) error {
	...
	rows, err := tx.NamedQuery(`
		INSERT INTO mandados (numero, lote, nome, documento, tipo_documento, sexo, posicao, endereco, cidade,
		                      situacao, data_carga, tipo_ato, diligencias, is_pj, repr_nome, repr_doc, obs, atos, motivo_nao_realizacao_id, updated_at)
		VALUES (:numero, :lote, :nome, :documento, :tipo_documento, :sexo, :posicao, :endereco, :cidade,
		        :situacao, :data_carga, :tipo_ato, :diligencias, :is_pj, :repr_nome, :repr_doc, :obs, :atos, :motivo_nao_realizacao_id, NOW())
		ON CONFLICT (numero) DO UPDATE SET
			lote           = EXCLUDED.lote,
			nome           = EXCLUDED.nome,
			documento      = EXCLUDED.documento,
